...
def update_backward_euler(num_t_values, num_x_values, u, dt, A, a, b):
    ...
    """
    #Converts the matrix to a compact representation that is used for solving fast.
    A = sc.sparse.csc_matrix(A)
    for i in range(1, num_t_values):
        #Uses scipy sparses functionality to solve tridiagonal systems fast.
        u[i,:] = spsolve(A , u[i-1, :] )
        u[i,-1] = b(1)
        u[i,0] = a(1)
    return u
...
def update_cn(num_t_values, num_x_values, u, alpha, A, a, b):
    ...
    # Compact represetnation of temp 1
    temp1 = sc.sparse.csc_matrix(temp1)

    temp2 = 2*np.eye(num_x_values) - alpha*A
    temp2[0,0] = temp2[-1,-1] = 1
    temp2[0,1] = temp2[-1,-2] = 0
    #Compact representation of temp 2
    temp2 = sc.sparse.csr_matrix(temp2)

    for i in range(1, num_t_values):
        #Does a forward step. Since temp2 is a csr matrix, the computations is done by using the fact that the matrix is tridiagonal
        forward_step = temp2.dot(u[i-1,:])
        #Solves the tridiagonal problem fast with spsolve.
...
def iterate(num_t_values, num_x_values, u_prev, dt, dx, dy):
    ...
    for the forward euler method in two dimensions. Uses numba to speed up.
